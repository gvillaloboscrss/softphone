...
class Chat(gui.ChatObserver):
	def __init__(self, app, acc, uri, call_inst=None):
		...
		self._gui = gui.ChatFrame(self, self._app, self._call)
	...
	def registerCall(self, uri_str, call_inst):
		uri = ParseSipUri(uri_str)
		try:
			print(f"Trying to find index for URI: {uri}")
			try:
				idx = self._participantList.index(uri)
			except ValueError:
				print(f"URI {uri} not found in participant list, adding it.")
			...
			try:
				...
				bud = self._buddyList[idx]
			print(f"Buddy found: {bud.cfg.uri}")
			self._callList.append(call_inst)
			print(f"Call instance set for index {idx}")
			call_inst.chat = self
			print("Chat instance set for call")
			call_inst.peerUri = bud.cfg.uri
			print(f"Peer URI set for call: {call_inst.peerUri}")
		except:
