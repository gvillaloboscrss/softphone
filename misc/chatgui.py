...
class ChatObserver:
    ...
    def onStartRec(self):
        global wav_writer, audio_media
        wav_writer = pj.AudioMediaRecorder()
        wav_writer.createRecorder(OUTPUT_FILE_PATH)
        print("audDevManager...")
        #mic_media = self._app.ep.audDevManager().getCaptureDevMedia()
        print("Recording...")
        try:
            ci = call.getInfo()
            ...
            for media in ci.media:
                if media.type == pj.PJMEDIA_TYPE_AUDIO:
                    try:
                        ...
                        aud_dev_mgr = self._app.ep.audDevManager()
                        audio_media.startTransmit(aud_dev_mgr.getPlaybackDevMedia())
                        audio_media.startTransmit(wav_writer)
                        aud_dev_mgr.getCaptureDevMedia().startTransmit(audio_media)
                    except pj.Error as e:
                        print(f"Error handling audio media: {e}")
        except Exception as e:
            print(f"Error retrieving call info: {e}")
        #mic_media.start_transmit(wav_writer)
    ...
    def onStopRec(self):
        ...
        global wav_writer, audio_media
        if wav_writer and audio_media:
            audio_media.stopTransmit(wav_writer)
            wav_writer = None
